        ...
        <div class="row">
            <div class="col-12">
                <div class="card">
                    ...
                    <!-- /.card-header -->
                    <form action="{{ route('admin.knowledge-base.update', $article->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PATCH') {{-- Menggunakan metode PATCH untuk update --}}
                        @php
                            $contentForEdit = old('content', $article->content);
                            $contentForEdit = preg_replace('/<div[^>]*data-kb-featured="1"[^>]*>.*?<\/div>\s*/is', '', $contentForEdit) ?? $contentForEdit;
                        @endphp
                        <div class="card-body">
                            ...
                            <div class="form-group">
                                ...
                                @if ($article->featured_image_url)
                                    <div class="mb-2">
                                        <img src="{{ $article->featured_image_url }}" alt="Featured Image {{ $article->title }}" class="img-thumbnail" style="max-width: 240px;">
                                    </div>
                                @endif
                            ...
                            <div class="form-group">
                                ...
                                <textarea id="summernote" name="content" class="form-control @error('content') is-invalid @enderror">{{ $contentForEdit }}</textarea>
                                @error('content')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
            ...
            // Video popover for post-insert controls (align, resize, edit, remove)
                ...
                $(document).on('keyup mouseup', function(e) {
                    ...
                    if (!$(e.target).closest('iframe').length) {
                        // small delay to allow other click handlers (like showing the popover)
                        setTimeout(() => {
